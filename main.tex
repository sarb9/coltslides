...
\begin{frame}{Upper bounds -- analysis}
    ...
    \begin{itemize}
        ...
        \item Let $(X, F) \sim \pi \otimes \xi$ and $\bar{f} = \E[F]$.
        \item Define
              \begin{align*}
                  \Delta(\pi, \xi) = \E[\bar{f}(X) - F(x_{F})]
                  \quad \text{and} \quad \cI(\xi) = \E[(F(X) - \bar{f}(X))^2]\,.
              \end{align*}
        \item If $\pi_\ts$ is played, then $(X, F) = (x_{F_1}, F_2)$, where $(F_1, F_2) \sim \xi \otimes \xi$.
    \end{itemize}
    \begin{tcolorbox}[title=Generalized $\IR$ for $\ts$,colback=green!5!white,colframe=green!50!black]
        Define generalized information associated with \alert{$\ts$} as the set of pairs
        \[
            \IR_\ts(\cF) = \left\{(\alpha, \beta) \in \R_+^2 : \Delta(\pi_\ts, \xi) \leq \alpha + \sqrt{\beta \cI(\pi_\ts, \xi)}  \;,\forall \xi\in \cP(\cF) \right\} \,,
        \]
    \end{tcolorbox}
    \begin{itemize}
        \item $(0,\beta) \in \IR_\ts(\cF) \Rightarrow \Delta(\pi_\ts, \xi)^2/\cI(\pi_\ts, \xi) \leq \beta$.
    \end{itemize}
    \only<2->{
        \begin{tikzpicture}[remember picture, overlay]
            \draw[red, ultra thick, rounded corners]
            ([yshift=2pt]$(current page.center)+(-5.5,-2)$)
            rectangle
            ([yshift=-2pt]$(current page.center)+(-4,-2.7)$);
            \node[anchor=north, red, fill=red!10!white] at
            ([yshift=2pt]$(current page.center)+(-4.75,-2.9)$)
            {only a function of $\cF$};
        \end{tikzpicture}
    }
\end{frame}
...
\begin{frame}{Upper bounds -- regret \&\ $\IR$}
    \small
    \begin{tcolorbox}[title=Proposition 1 -- regret bound through $\IR_\ts$,colback=blue!5!white,colframe=blue!50!black]
        Suppose that $\cF \in \{\cF_{\pb\pl}, \cF_{\pb\pl\pr\pm}\}$ and $(\alpha, \beta) \in \IR_\ts(\cF)$.
        Then, the regret of $\ts$ is at most
        \begin{align*}
            \BReg_n(\ts, \xi_0) \leq n \alpha + O\left(\sqrt{\beta n d \log(n \diam(\cK))}\right) \,.
        \end{align*}
    \end{tcolorbox}
    \only<2-2>{It remains to find a pair $(\alpha, \beta) \in \IR_\ts$.}
    \uncover<3->{
        \begin{tcolorbox}[title=Proposition 2 -- $\IR_\ts$ Partitioning,colback=blue!5!white,colframe=blue!50!black]
            ...
            \end{align*}
            for all $i$, then $(\alpha, k(k-1)m\beta) \in \IR_\ts(\cF)$.
        \end{tcolorbox}
        \only<4->{
            \begin{tikzpicture}[remember picture, overlay]
                \draw[red, ultra thick, rounded corners]
